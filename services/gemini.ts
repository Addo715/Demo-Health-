...
export async function sendMessageToGemini(
  ...
  try {
    // Build chat history — skip the first welcome message
    const contents: GeminiMessage[] = history.slice(1).map((msg) => ({
      role: msg.role === 'user' ? 'user' : 'model',
    ...
    // Strip any stray asterisks or markdown as a safety net
    return reply.replace(/\*+/g, '').trim();

  } catch (error) {
    console.error('sendMessageToGemini error:', error);
    return 'Sorry, I was unable to process your request right now. Please try again.';
  }
}